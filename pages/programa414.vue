    ...
    <v-card>
      ...
      <v-data-table :headers="headers" :items="filteredReferrals" :loading="isLoading" item-value="$id" hover no-data-text="Aún no hay referidos cargados.">
        ...
        <template v-slot:item.status="{ item }">
          <v-select v-model="item.status" :items="statusOptions" @update:modelValue="(newStatus) => updateReferralStatus(item.$id, newStatus as IReferral['status'])" density="compact" hide-details variant="outlined" :bg-color="getStatusColor(item.status)" class="status-select"></v-select>
        </template>
      </v-data-table>
    </v-card>
  </div>
...
const headers = [
  { title: 'Fecha de Carga', key: 'loadDate' }, { title: 'Sponsor', key: 'sponsor' }, { title: 'Referido', key: 'referralName' }, { title: 'Teléfono', key: 'phone' },
  { title: 'Ocupación', key: 'occupation' }, { title: 'Estado', key: 'status', width: '200px' },
];
...
// ✅ --- FUNCIÓN DE GUARDADO MODIFICADA CON ALERTA DE DIAGNÓSTICO ---
const handleSaveAllReferrals = async () => {
  ...
  for (const referral of newReferralsList.value) {
    try {
      ...
      alert(`ERROR AL GUARDAR A "${referral.referralName}":\n\n${error.message}`);
      isSaving.value = false; // Detenemos la carga si hay un error
      return; 
    }
  }
  
  // Si todo salió bien, refrescamos y limpiamos el formulario
  if (successfulSaves > 0) {
    await getReferrals();
    // Reemplazamos la notificación de snackbar por una alerta para que la veas seguro
    alert(`${successfulSaves} referidos guardados con éxito.`);
  }
