...
const actionableGoals = computed(() => {
  ...
  const WORK_DAYS_IN_WEEK = 5; // Asumimos 5 días hábiles
  const weeklySalesTarget = (projection.value.projectedSalesCount / WEEKS_IN_MONTH).toFixed(1);
  const dailyContactTarget = Math.ceil(projection.value.projectedReferralsToContact / (WEEKS_IN_MONTH * WORK_DAYS_IN_WEEK));
  return { weeklySalesTarget, dailyContactTarget };
});
...
const projectedProductFocus = computed(() => {
  ...
  const productFocus = productContributions.map(product => {
    ...
    return {
      ...
      currentCount: Math.round(product.totalValue / (productPrice || 1)),
    };
  });
